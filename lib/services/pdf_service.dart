...
class PDFservice
{
      ...
      }) async
  {
    ...
    final List<Map<String, String>> immunisations =
    immunisationQuery.docs.map((doc)
    {
      final data = doc.data();
      final name = (data['name'] ?? 'Unknown').toString();
      final datesList = (data['dates'] as List?) ?? [];
      final dates = datesList.map((d) => _formatDoseText(d.toString())).join(', ');
      return {
        'name': name,
    ...
    // map vaccine name to latest recorded date
    final Map<String, String> latestByVaccine =
    {
    };
    ...
    for (final item in immunisations)
    {
      ...
      latestByVaccine[name] = _formatDoseText(parts.last.trim());
    }
                  ...
                  children: [
                    ...
                    pw.Table(
                      ...
                      children: [
                        pw.TableRow(
                          ...
                          children: [
                            pw.Padding(
                              ...
                              child: pw.Text("Vaccine",
                                  style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                            ),
                            pw.Padding(
                              padding: pw.EdgeInsets.all(8),
                              child: pw.Text("Date given",
                                  style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                            ),
                          ],
                        ),
                        ...
                        {
                          final date = latestByVaccine[vaccine] ?? '';
                          final done = date.isNotEmpty;

                          return pw.TableRow(
                            decoration: pw.BoxDecoration(
                              color: done ? PdfColors.green100 : PdfColors.red100,
                            ),
                            children: [
                              pw.Padding(
                                padding: pw.EdgeInsets.all(8),
  ...
  static String _formatDoseText(String input)
  {
    final match = RegExp(r'\(dose\s*(\d+)\)', caseSensitive: false).firstMatch(input);
    if (match == null)
    {
      return input;
    }
    ...
    return input.replaceAll(match.group(0)!, "(${d}${suffix} dose)");
  }
